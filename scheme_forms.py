...
def do_and_form(expressions, env):
    ...
    if is_scheme_false(evaluated_expression):
        return False
...
def do_or_form(expressions, env):
    ...
    if not expressions:
        return False
    evaluated_expression = scheme_eval(expressions.first, env)
    if is_scheme_true(evaluated_expression) or expressions.rest is nil:
        return evaluated_expression
...
def make_let_frame(bindings, env):
    ...
    values = Pair(eval_all(bindings.first.rest, env), nil)
    while bindings.rest:
        bindings = bindings.rest
        validate_form(bindings.first, 2, 2)
        name, value = bindings.first.first, eval_all(bindings.first.rest, env) 
        names.rest = Pair(name, names.rest)
        values.rest = Pair(value, values.rest)
